...
![](C:\JavaEE_Relation\JavaEE54\day32_增删改查练习&分页\笔记\img\temp5.png)
...
public class PageBean {
    private List<Contactor> list;//当前页数据集合
...
/**
    ...
    public PageBean findPageBean(Long currentPage) {
        ...
        //2.3 封装totalSize
        //调用dao层的方法，到数据库查询总数据条数
        Long totalSize = userDao.findContactorCount();
        pageBean.setTotalSize(totalSize);
        //2.4 封装totalPage,要考虑除不尽的情况
        Long totalPage = totalSize % pageSize == 0 ? totalSize/pageSize : totalSize/pageSize + 1;
        pageBean.setTotalPage(totalPage);
        //2.5 封装list
        //调用dao层的方法查询当前页的数据集合
        List<Contactor> list = userDao.findPageContactors(currentPage,pageSize);
